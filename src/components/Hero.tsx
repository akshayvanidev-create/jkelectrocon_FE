...
const SLIDE_DATA = [
  {
    ...
    subtitle:
      "Architecting high-fidelity smart living for the next generation.",
  },
  {
    image:
    ...
    subtitle:
      "Experience studio-grade acoustics in the comfort of your sanctuary.",
  },
  {
    image:
      "https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?auto=format&fit=crop&q=80&w=2400",
    title: "Seamless Orchestration.",
    subtitle:
      "Complete control of your environment with a single intuitive touch.",
  },
  {
    image:
      "https://images.unsplash.com/photo-1600607687644-c7171b42498f?auto=format&fit=crop&q=80&w=2400",
    title: "Invisible Intelligence.",
    subtitle:
      "Advanced ERV & purification systems for a healthier, purer home.",
  },
];
    ...
    >
      ...
      <div className="relative z-10 text-center px-4 max-w-6xl">
        <div ref={badgeRef} className="mb-4 inline-block opacity-0">
          <span className="px-4 py-1.5 rounded-full border border-button bg-button/10 backdrop-blur-md text-[10px] font-black uppercase tracking-[0.4em] text-hover">
            Intelligence redefined
          </span>
        </div>
        <h1
          ref={titleRef}
          className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl font-extrabold mb-4 md:mb-6 tracking-tighter text-white leading-[1.05] opacity-0 drop-shadow-2xl px-2"
        >
          {SLIDE_DATA[currentSlide].title.split(" ").map((word, i, arr) => (
            <React.Fragment key={i}>
              {i === arr.length - 1 ? (
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-button to-hover italic font-light">
                  {word}
                </span>
              ) : (
                word + " "
        ...
        <div className="relative inline-block mb-8 md:mb-10 max-w-3xl mx-auto px-2">
          <div className="absolute inset-0 bg-card/80 blur-xl rounded-2xl"></div>
          <p
            ref={paraRef}
            className="relative text-lg sm:text-xl md:text-2xl lg:text-3xl text-text font-normal tracking-wide px-4 md:px-6 py-3 md:py-4 opacity-0 drop-shadow-lg"
          >
        ...
        <div
          ref={btnRef}
          className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center items-center opacity-0 px-2"
        >
          <a
            href="#portfolio"
            className="group relative px-8 md:px-10 py-4 md:py-5 text-base md:text-lg font-bold text-background bg-button rounded-full overflow-hidden transition-all duration-300 shadow-2xl shadow-button/20"
          >
        ...
        {SLIDE_DATA.map((_, i) => (
          <div
            ...
            className={`h-1 transition-all duration-700 rounded-full ${
              i === currentSlide ? "w-12 bg-button" : "w-6 bg-card"
            }`}
          ></div>
        ))}
      </div>
      ...
      <div className="absolute bottom-10 left-10 hidden md:block z-20">
        <div className="flex flex-col items-center gap-4">
          <div className="w-[1px] h-20 bg-gradient-to-b from-button via-card/10 to-transparent"></div>
        </div>
      </div>
    </section>
  );
